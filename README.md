# PDF Highlight

A Preview-style highlighter for research PDFs. Open a file, mark text with custom RGB colors, add notes, and download an annotated PDF. Every edit is saved in the browser.

Live: [https://aaronyy.github.io/pdfhighlight/](https://aaronyy.github.io/pdfhighlight/)

## Local

```bash
npm i
npm run dev
```

The Vite base path is `/pdfhighlight/`, so the app is at `http://localhost:5173/pdfhighlight/`.
